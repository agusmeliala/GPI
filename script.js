...
container.innerHTML = daftarJadwal.map(j => `
    <div class="flex justify-between items-center py-2 border-b border-slate-700/50 last:border-0">
        <div class="flex flex-col">
            <span class="text-[10px] font-bold text-blue-400 uppercase tracking-wider mb-0.5">${j.kategori}</span>
            <h4 class="text-white font-semibold text-base leading-tight">${j.nama}</h4>
            ${j.keterangan ? `<p class="text-slate-400 text-xs mt-0.5 leading-none">${j.keterangan}</p>` : ''}
        </div>
        <div class="text-right flex flex-col items-end justify-center">
            <span class="text-white font-bold text-base leading-none">${j.jam}</span>
            <span class="text-[9px] text-slate-500 font-medium uppercase mt-1">WIB</span>
        </div>
    </div>
`).join('');
